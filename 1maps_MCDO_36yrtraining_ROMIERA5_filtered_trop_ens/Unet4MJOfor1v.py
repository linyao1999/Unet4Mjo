...
# output: ROMIERA5 at one time step
...
Fn1 = datadir+'ERA5.u200G4ROMI.day.1979to2021.nc'
Fn2 = datadir+'ERA5.u850G4ROMI.day.1979to2021.nc'
Fn3 = datadir+'ERA5.olrG4ROMI.day.1979to2021.nc'

Fn4 = datadir+'ERA5.tcwvG4ROMI.day.1979to2021.nc'
Fn5 = datadir+'ERA5.v200G4ROMI.day.1979to2021.nc'
Fn6 = datadir+'ERA5.T200G4ROMI.day.1979to2021.nc'
Fn7 = datadir+'ERA5.prepG4ROMI.day.1979to2021.nc'

Fn8 = datadir+'ERA5.u500G4ROMI.day.1979to2021.nc'
Fn9 = datadir+'ERA5.v500G4ROMI.day.1979to2021.nc'
Fn10 = datadir+'ERA5.v850G4ROMI.day.1979to2021.nc'
Fn11 = datadir+'ERA5.Z200G4ROMI.day.1979to2021.nc'
Fn12 = datadir+'ERA5.Z500G4ROMI.day.1979to2021.nc'
Fn13 = datadir+'ERA5.Z850G4ROMI.day.1979to2021.nc'
Fn14 = datadir+'ERA5.T500G4ROMI.day.1979to2021.nc'
Fn15 = datadir+'ERA5.T850G4ROMI.day.1979to2021.nc'
Fn16 = datadir+'ERA5.q200G4ROMI.day.1979to2021.nc'
Fn17 = datadir+'ERA5.q500G4ROMI.day.1979to2021.nc'
Fn18 = datadir+'ERA5.q850G4ROMI.day.1979to2021.nc'
...
Fnmjo = '/global/homes/l/linyaoly/ERA5/reanalysis/cal_ROMI/ROMI_ERA5_daily_1979to2021.nc'
### PATHS and FLAGS ###
...
path_forecasts = './ROMI_dp0102_1dmem_' + varn + '/'
...
num_samples = 2
lambda_reg = 0.2
knlat = 21 # added by lin 91->2deg # Nlat changed to 1 for hovmoller forecast
...
# path_weights = '/global/homes/l/linyaoly/ERA5/script/Stability-Explanability/U-NET/weights_analysis/'
# # path_forecasts = '/global/homes/l/linyaoly/ERA5/script/Stability-Explanability/U-NET/forecasts_2deg/'
# path_forecasts = '/global/homes/l/linyaoly/ERA5/script/Stability-Explanability/7maps_36yrtraining_ROMIERA5_filtered_trop/output/'
...
net = net.eval()
# torch.save(net.state_dict(), model_save+'predicted_MCDO_UNET_7mapstrop_ROMIERA5_36yr_lead'+str(leadmjo)+'_dailyinput.pt')
...
for testyn in np.arange(testystat,testyend):
        ...
        autoreg_pred_ctl = np.zeros((M,2)) # control forecast with unperturbed IC; without MCDO 
        Nsamp = 100  # ensemble number
        autoreg_pred_dis = np.zeros((M,Nsamp,2))  # ensemble forecast with perturbed ICs (step, # in the ensemble, ROMI index)
        # autoreg_pred_std_ctl = np.zeros((M,2))
        autoreg_true = np.zeros((M,2))

        for k in range(0,M):
                # control forecast input at one time step
                inputx = psi_test_input_Tr_torch[k].reshape([1,nmaps*nmem,dimx,dimy]).cuda()

                # # # add random noise to normalized input; factor is 0.3*std at a time step
                STDinputx = torch.std(torch.flatten(inputx)) 
                # # # 
                print('STDinputx: '+str(STDinputx.data.cpu()))
                inputxptb = STDinputx * torch.randn(Nsamp,nmaps*nmem,dimx,dimy).cuda() + inputx 
                # net = net.train()
                # out_dis = np.array([net(inputx).data.cpu().numpy() for _ in range(Nsamp)]).squeeze()
                net = net.eval()
                out = net(inputx).data.cpu().numpy()
                net = net.eval()
                out_dis = np.array([net(inputxptb[ic].reshape([1,nmaps*nmem,dimx,dimy]).cuda()).data.cpu().numpy() for ic in range(Nsamp)]).squeeze()
                # out = (net(psi_test_input_Tr_torch[k].reshape([1,nmaps*nmem,dimx,dimy]).cuda())) # Nlat changed to 1 for hovmoller forecast
                # net = net.train()                
                print('out_dis shape: '+str(out_dis[None,:,:].shape))
                autoreg_pred_dis[k,None,:,:] = out_dis[None,:,:]
                # autoreg_pred_std[k,None,:] = out_dis.std(axis=0)
                # autoreg_pred[k,None,:] = (out.detach().cpu().numpy())
                autoreg_pred_ctl[k,None,:] = out
        ...
        print('autoreg_pred_ctl' + str(autoreg_pred_ctl[0,0]))
        print('autoreg_pred_ctl' + str(autoreg_pred_ctl.shape))

        np.savetxt(path_forecasts+'predicted_ctl_MCDO_UNET_'+varn+'trop_ROMIERA5_36yr_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d'+str(testyn)+'.csv', autoreg_pred_ctl, delimiter=",")
        np.savetxt(path_forecasts+'truth_MCDO_UNET_'+varn+'trop_ROMIERA5_36yr_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d'+str(testyn)+'.csv', autoreg_true, delimiter=",")
        # np.savetxt(path_forecasts+'predicted_ens_MCDO_UNET_'+varn+'trop_ROMIERA5_36yr_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d'+str(testyn)+'.csv', autoreg_pred_std, delimiter=",")
        
        ds = xr.Dataset(
                data_vars=dict(
                        ROMI=(["timestp","N","mode"], autoreg_pred_dis),
                ),
                coords=dict(
                        timestp = np.arange(M),
                        N = np.arange(Nsamp),
                ...
                attrs=dict(
                        description="IC forecast ensemble"
                ),
                )

        ds.to_netcdf(path_forecasts+'predicted_ens_MCDO_UNET_'+varn+'trop_ROMIERA5_36yr_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d'+str(testyn)+'.nc')
        # savenc(autoreg_pred, path_forecasts+'predicted_UNET_u200u850olr_ROMIERA5_lead'+str(leadmjo)+'.nc')
        # savenc(autoreg_true, path_forecasts+'truth_UNET_u200u850olr_ROMIERA5_lead'+str(leadmjo)+'.nc')
