...
def load_test_data(Fnmjo,leadmjo,mem_list,testystat,testyend,zmode,m,lat_lim):
    # set parameters
    nmem = len(mem_list)  # memory length
    dimx = 1
    dimy = 180

    # make projection  (time, 1, lon)
    olr_re = projection(zmode, m, [str(testystat)+'-01-01', str(testyend-1)+'-12-31'], [lat_lim,-lat_lim])
...
def load_train_data(Fnmjo,leadmjo,mem_list,ysta,yend,zmode,m,lat_lim):
    # set parameters
    nmem = len(mem_list)  # memory length
    dimx = 1
    dimy = 180
    ...
    olr_re = projection(zmode, m, [str(ysta)+'-01-01', str(yend-1)+'-12-31'], [lat_lim,-lat_lim])
