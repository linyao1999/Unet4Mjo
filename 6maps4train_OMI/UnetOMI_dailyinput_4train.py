...
from data_loaderomi_dailyinput_4train import load_test_data
from data_loaderomi_dailyinput_4train import load_train_data
...
trainN = 345
M = 365  
leadmjo = int(os.environ["lead4train"]) # lead for output (the MJO index)
nmem = int(os.environ["memlen4train"])  # the number of how many days we want to include into the input maps
mem_list = np.arange(nmem)

nmaps = 6  # the number of variables we include in the input
nmaptrain = 4  # the number of variables we include in the test
...
path_forecasts = '/global/homes/l/linyaoly/ERA5/script/Stability-Explanability/6maps4train_OMI/output/'
...
for testyn in np.arange(testystat,testyend):
        ...
        for leveln in np.arange(0,2*nmem):
                M_test_level = torch.mean(torch.flatten(psi_test_input_Tr_torch[:,leveln,:,:]))
                STD_test_level = torch.std(torch.flatten(psi_test_input_Tr_torch[:,leveln,:,:]))
                print('leveln: '+str(leveln))
                psi_test_input_Tr_torch_norm[:,leveln,None,:,:] = ((psi_test_input_Tr_torch[:,leveln,None,:,:]-M_test_level)/STD_test_level)
        
        for leveln in np.arange(4*nmem,nmaps*nmem):
                M_test_level = torch.mean(torch.flatten(psi_test_input_Tr_torch[:,leveln,:,:]))
                STD_test_level = torch.std(torch.flatten(psi_test_input_Tr_torch[:,leveln,:,:]))
                print('leveln: '+str(leveln))
                psi_test_input_Tr_torch_norm[:,leveln,None,:,:] = ((psi_test_input_Tr_torch[:,leveln,None,:,:]-M_test_level)/STD_test_level)

        # psi_test_input_Tr_torch_norm[:,] = 0.
        psi_test_input_Tr_torch  = torch.from_numpy(psi_test_input_Tr_torch_norm).float()
        ...
        np.savetxt(path_forecasts+'predicted_UNET_u200u850olrprepv200T200_OMI_lead'+str(leadmjo)+'_dailyinput_4train_mem'+str(nmem)+'d'+str(testyn)+'.csv', autoreg_pred, delimiter=",")
        np.savetxt(path_forecasts+'truth_UNET_u200u850olrprepv200T200_OMI_lead'+str(leadmjo)+'_dailyinput_4train_mem'+str(nmem)+'d'+str(testyn)+'.csv', autoreg_true, delimiter=",")
