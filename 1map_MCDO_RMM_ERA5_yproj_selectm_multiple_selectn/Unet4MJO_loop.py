...
model_save = '/pscratch/sd/l/linyaoly/Unet4MJO/1map_MCDO_RMM_yproj_zonalfft/'
...
psi_train_input_Tr_torch, psi_train_label_Tr_torch, trainN = load_train_data(vn,Fnmjo,leadmjo,mem_list,ysta,yend,zmode,m,mflg,wnx,wnxflg,lat_lim)
...
psi_test_input_Tr_torch, psi_test_label_Tr_torch, psi_test_label_Tr, M  = load_test_data(vn,Fnmjo,leadmjo,mem_list,testystat,testyend,zmode,m,mflg,wnx,wnxflg,lat_lim)
...
net = net.eval()
torch.save(net.state_dict(), model_save+'predicted_MCDO_UNET_1map'+str(lat_lim)+'deg_RMM_ERA5_lead'+str(leadmjo)+'_dailyinput_m'+str(m)+mflg+'_wnx'+str(wnx)+wnxflg+'_zmode'+str(zmode)+'_nmem'+str(nmem)+'.pt')
...
ds.to_netcdf(path_forecasts+'predicted_MCDO_UNET_olr'+str(lat_lim)+'deg_RMMERA5_'+str(m)+'modes'+mflg+'_wnx'+str(wnx)+wnxflg+'_lead'+str(leadmjo)+'_dailyinput_'+str(ysta)+'to'+str(yend)+'_mem'+str(nmem)+'d.nc')
