...
def load_test_data(vn, Fnmjo,leadmjo,mem_list,testystat,testyend,zmode,m,mflg,wnx,wnxflg,lat_lim):
    # set parameters
    ...
    olr_re = projection(vn, zmode, m, mflg, wnx, wnxflg, [str(testystat)+'-01-01', str(testyend-1)+'-12-31'], [lat_lim,-lat_lim])
...
def load_train_data(vn,Fnmjo,leadmjo,mem_list,ysta,yend,zmode,m,mflg,wnx,wnxflg,lat_lim):
    # set parameters
    nmem = len(mem_list)  # memory length
    dimx = int(1 + 2 * int(lat_lim / 2))
    dimy = 180
    ...
    # make projection and do zonal fft (time, lat, lon)
    olr_re = projection(vn, zmode, m, mflg, wnx, wnxflg, [str(ysta)+'-01-01', str(yend-1)+'-12-31'], [lat_lim,-lat_lim])
