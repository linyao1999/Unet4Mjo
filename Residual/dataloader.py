...
def load_train_data(leadmjo,ysta=1979,yend=2014,nmem=300,nmemUnet=1,nvar=3,noutputmem=1,normflg=1):
    ...
    # slice the training data at t+lead to calculate residual of truth - prediction
    dstmp1 = dsrmmt.loc[dict(time=slice((datesta + np.timedelta64(nmem-1+leadmjo-noutputmem+1,'D')), (dateend1 + np.timedelta64(leadmjo,'D'))))]
    RMMt1 = dstmp1['RMM'][:,0]  # (day)
    RMMt2 = dstmp1['RMM'][:,1]  # (day)

    # slice the training data at t+lead to calculate residual of truth - prediction
    dstmp2 = dsrmmp.loc[dict(time=slice((datesta + np.timedelta64(nmem-1-noutputmem+1,'D')), dateend1))]
    RMMp1 = dstmp2['RMM'][:,0]  # (day)
    RMMp2 = dstmp2['RMM'][:,1]  # (day)
    ...
    for i in range(Ntrain):
        train_model_label[i,0,:] = resi1o[i:i+noutputmem]
        train_model_label[i,1,:] = resi2o[i:i+noutputmem]
...
def load_test_data(leadmjo,ysta=2015,yend=2018,nmem=300,nmemUnet=1,nvar=3,noutputmem=1,normflg=1):
    ...
    # add memories to the model label
    for i in range(Ntest):
        test_model_label[i,0,:] = resi1o[i:i+noutputmem]
        test_model_label[i,1,:] = resi2o[i:i+noutputmem]

    if normflg:
        # normalize input_data and labels
        test_input_data_norm = np.empty((Ntest, nvar*2, nmem))  # (Nsamples, channels, nmem)
        test_model_label_norm = np.empty((Ntest, 2, noutputmem))  # (Nsampels, channels, nmem)
