...
Fnmjo = '/global/homes/l/linyaoly/ERA5/reanalysis/omi/'
...
num_samples = 2
lambda_reg = 0.2
knlat = 15 # added by lin 15->2deg # Nlat changed to 1 for hovmoller forecast
...
path_forecasts = '/global/homes/l/linyaoly/ERA5/script/Stability-Explanability/7maps_36yrtraining_OMI_filtered_trop/output/'
...
#        output,_,_,_,_,_,_ = net(input_batch.cuda())
        ...
        loss.backward()
        optimizer.step()
        output_val= net(psi_test_input_Tr_torch[0:num_samples].reshape([num_samples,nmaps*nmem,15,180]).cuda()) # Nlat changed to 1 for hovmoller forecast
...
for testyn in np.arange(testystat,testyend):
        ...
        for k in range(0,M):
                out = (net(psi_test_input_Tr_torch[k].reshape([1,nmaps*nmem,15,180]).cuda())) # Nlat changed to 1 for hovmoller forecast
        ...
        # out = (net(torch.from_numpy(((autoreg_pred[k-1,:,:,:])).reshape([1,3,15,180])).float().cuda())) # Nlat changed to 1 for hovmoller forecast
        ...
        np.savetxt(path_forecasts+'predicted_UNET_7mapstrop_OMI_36yr_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d'+str(testyn)+'.csv', autoreg_pred, delimiter=",")
        np.savetxt(path_forecasts+'truth_UNET_7mapstrop_OMI_36yr_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d'+str(testyn)+'.csv', autoreg_true, delimiter=",")
