...
def load_test_data(Fn1,Fn2,Fn3,Fn4,Fn5,Fn6,Fnmjo,leadmjo,mem_list):
  ...
  psi1=np.asarray(FF1['u'])  # u200
  psi2=np.asarray(FF2['u'])  # u850
  psi3=np.asarray(FF3['ttr'])  # olr
  psi4=np.asarray(FF4['tp'])  # prep
  ...
  psi_test_label = pc[mem_list[-1]+leadmjo*4:mem_list[-1]+leadmjo*4+ndays,:]
...
def load_train_data(loop1,loop2,loop3,loop4,loop5,loop6,loopmjo,leadmjo,mem_list):     ##### change the rest of the stuff in train
     ...
     # calculate daily average
     psi1=np.asarray(File1['u'])  # u200
     psi2=np.asarray(File2['u'])  # u850
     psi3=np.asarray(File3['ttr'])  # olr
     psi4=np.asarray(File4['tp'])  # prep
     psi5=np.asarray(File5['v'])  # v200
     psi6=np.asarray(File6['t'])  # T200
     ...
     psi_train_label = pc[mem_list[-1]+leadmjo*4:mem_list[-1]+leadmjo*4+ndays,:] 
