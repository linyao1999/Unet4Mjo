...
from dataloader_pred import load_test_data
from dataloader_pred import load_train_data

########### Goals ###############
# create a model using both a LSTM layer and FC layers
# to predict the RMM
...
normflg = 1  # whether normalize the input data
num_epochs = 100  # loop time
batch_size = 64
nvar = 1  # how many variables used as input
nhidden1 = 10  # nodes in the linear hidden layer 1
flg = str(nvar)+'vari_predasinput_'
...
# ################# define LSTM_NN ##############
class LSTM_NNnet(nn.Module):
    def __init__(self, batch_first=True):
        super().__init__()
        self.lstm = nn.LSTM(input_size=2*nvar, hidden_size=2*noutputmem)
        self.FC1 = nn.Linear(2*noutputmem,nhidden1)
        self.FC2 = nn.Linear(nhidden1,2*noutputmem)
    ...
    def forward(self, input):
        ...
        # print('hn shape: ', lstm_out.shape)

        x1 = lstm_out[:,-1,:].view(-1,2*noutputmem) # (1,batch_size,2)
        x2 = F.relu(self.FC1(x1))

        prediction = self.FC2(self.dropout(x2))
...
print('loss is:', loss_values[-20:])
net = net.eval()
torch.save(net.state_dict(), model_save+'LSTM_NN_MCDO_UNET_'+flg+'19mapstrop_RMMERA5_36yr_lead'+str(leadmjo)+'_dailyinput_'+str(nmem)+'dmem.pt')
...
resi = xr.DataArray(
        ...
        attrs=dict(
                description='RMM prediction at time, already including lead'
        ),
)

ds = resi.to_dataset(name='RMM')
ds.to_netcdf(path_forecasts+'LSTM_NN_MCDO_UNET_'+flg+'RMMERA5_36yr_lead'+str(leadmjo)+'_norm'+str(normflg)+'_dailyinput_nmem'+str(nmem)+'nout'+str(noutputmem)+'d.nc')
