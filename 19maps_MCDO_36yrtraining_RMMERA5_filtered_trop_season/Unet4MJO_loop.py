...
# updated on April 04 2023: train model only using data from selected months
...
# parameters below set the season we want to train the model
mon1 = int(os.environ["mon1"])  # from which month the model starts to use
monlen = int(os.environ["monlen"])  # how many months in a year used in the model  
...
num_epochs = int(os.environ["numepo"])   # how many loops we want to train the model. 
...
model_save = '/pscratch/sd/l/linyaoly/Unet4MJO/19variables_MCDO_36yr_filtered_trop_season/'
...
##### prepare test data ###################################################
psi_test_input_Tr_torch, psi_test_label_Tr_torch, psi_test_label_Tr  = load_test_data(Fn,vn,Fnmjo,leadmjo,mem_list,testystat,lat_lim,mon1,monlen)
...
for epoch in range(0, num_epochs):  # loop over the dataset multiple times
    ...
    for yn in np.arange(ysta,yend):
        ...
        psi_train_input_Tr_torch, psi_train_label_Tr_torch, trainN = load_train_data(Fn,vn,Fnmjo,leadmjo,mem_list,yn,lat_lim,mon1,monlen)
...
net = net.eval()
torch.save(net.state_dict(), model_save+'predicted_MCDO_UNET_'+str(len(vn))+'mapstrop_RMMERA5_36yr_lead'+str(leadmjo)+'_dailyinput_mon'+str(mon1)+'.pt')
...
for testyn in np.arange(testystat,testyend):
    ...
    psi_test_input_Tr_torch, psi_test_label_Tr_torch, psi_test_label_Tr  = load_test_data(Fn,vn,Fnmjo,leadmjo,mem_list,testyn,lat_lim,mon1,monlen)
    psi_test_input_Tr_torch_norm = np.zeros(np.shape(psi_test_input_Tr_torch))
    ...
    datesta = pd.Timestamp(np.datetime64(str(testyn-1)+'-12-01')) + pd.DateOffset(months=mon1) 
    tmp = datesta + pd.DateOffset(months=monlen)
    dateend = tmp.to_numpy() - np.timedelta64(1, 'D')
    datesta = datesta.to_numpy()
    delta = dateend - datesta
    M = int(delta / np.timedelta64(1, 'D')) + 1  
    ...
    t.append(np.arange(datesta, dateend+np.timedelta64(1, 'D'), dtype='datetime64[D]'))
...
ds.to_netcdf(path_forecasts+'predicted_MCDO_UNET_'+str(len(vn))+'mapstrop_RMMERA5_36yr_mon'+str(mon1)+'_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d.nc')
