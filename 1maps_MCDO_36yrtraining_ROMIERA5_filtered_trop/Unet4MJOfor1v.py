...
for testyn in np.arange(testystat,testyend):
        ...
        Nsamp = 100
        autoreg_pred_dis = np.zeros((M,Nsamp,2))  # (step, # in the ensemble, ROMI index)
        ...
        np.savetxt(path_forecasts+'predicted_MCDO_UNET_'+varn+'trop_ROMIERA5_36yr_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d'+str(testyn)+'.csv', autoreg_pred, delimiter=",")
        ...
        np.savetxt(path_forecasts+'predicted_disstd_MCDO_UNET_'+varn+'trop_ROMIERA5_36yr_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d'+str(testyn)+'.csv', autoreg_pred_std, delimiter=",")
        ...
        ds = xr.Dataset(
                ...
                attrs=dict(
                        description="MCDO forecast ensemble"
                ),
                )

        ds.to_netcdf(path_forecasts+'predicted_dis_MCDO_UNET_'+varn+'trop_ROMIERA5_36yr_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d'+str(testyn)+'.nc')
