...
def load_test_data(Fn,vn,Fnmjo,leadmjo,mem_list,yn,lat_lim,mon1,monlen):
    # set parameters
    nmem = len(mem_list)  # memory length

    datesta = pd.Timestamp(np.datetime64(str(yn-1)+'-12-01')) + pd.DateOffset(months=mon1) 
    ...
    dateend1 = dateend + np.timedelta64(leadmjo+nmem, 'D')
    ...
    for ivn in np.arange(len(vn)):
        ...
        FF0 = FF0.sel(time=slice(datesta, dateend1), lat=slice(lat_lim,-lat_lim))
        # fill the Nan
        FF0.fillna(0)
    ...
    FFmjo = FFmjo.sel(time=slice(datesta, dateend1))
    FFmjo.fillna(0)
...
def load_train_data(Fn,vn,Fnmjo,leadmjo,mem_list,yn,lat_lim,mon1,monlen):   ##### change the rest of the stuff in train
    # load training data
    # set parameters
    nmem = len(mem_list)  # memory length
    datesta = pd.Timestamp(np.datetime64(str(yn-1)+'-12-01')) + pd.DateOffset(months=mon1) 
