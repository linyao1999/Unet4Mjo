...
Fn4 = [datadir+'tcwv_2deg2015.nc', datadir+'tcwv_2deg2016.nc']
Fn5 = [datadir+'v200_2deg2015.nc', datadir+'v200_2deg2016.nc']
Fn6 = [datadir+'T200_2deg2015.nc', datadir+'T200_2deg2016.nc']
Fn7 = datadir+'ERA5.sstGfltGmask0.6hr.1979to2022.nc'
Fn = [Fn1,Fn2,Fn3,Fn4,Fn5,Fn6,Fn7]
Fnmjo = '/global/homes/l/linyaoly/ERA5/reanalysis/RMM_ERA5_6hr.nc'

num_epochs = 15   # how many loops we want to train the model. 
trainN = 345*4
M = 365*4  
leadmjo = int(os.environ["lead30d"]) # lead (time steps) for output (the MJO index)
nmem = int(os.environ["memlen"])  # the number of how many time steps we want to include into the input maps
mem_list = np.arange(nmem)

nmaps = 7  # the number of variables we include in the input
nmapsnorm = 6  # the number of variables need to be normalized
...
for epoch in range(0, num_epochs):  # loop over the dataset multiple times
    ...
    for yn in np.arange(ysta,yend):
     ...
     Fn4 = [datadir+'tcwv_2deg'+str(yn)+'.nc', datadir+'tcwv_2deg'+str(yn+1)+'.nc']
     Fn5 = [datadir+'v200_2deg'+str(yn)+'.nc', datadir+'v200_2deg'+str(yn+1)+'.nc']
     Fn6 = [datadir+'T200_2deg'+str(yn)+'.nc', datadir+'T200_2deg'+str(yn+1)+'.nc']
     Fn = [Fn1,Fn2,Fn3,Fn4,Fn5,Fn6,Fn7]
     print(Fn4)
     print(Fn5)
     print(Fn6)

     psi_train_input_Tr_torch, psi_train_label_Tr_torch = load_train_data(Fn,Fnmjo,leadmjo,mem_list,yn)
     del Fn
...
#        output,_,_,_,_,_,_ = net(input_batch.cuda())
        ...
        loss.backward()
        optimizer.step()
        output_val= net(psi_test_input_Tr_torch[0:num_samples].reshape([num_samples,nmaps*nmem,dimx,180]).cuda()) # Nlat changed to 1 for hovmoller forecast
...
for testyn in np.arange(testystat,testyend):
        ...
        Fn4 = [datadir+'tcwv_2deg'+str(testyn)+'.nc', datadir+'tcwv_2deg'+str(testyn+1)+'.nc']
        Fn5 = [datadir+'v200_2deg'+str(testyn)+'.nc', datadir+'v200_2deg'+str(testyn+1)+'.nc']
        Fn6 = [datadir+'T200_2deg'+str(testyn)+'.nc', datadir+'T200_2deg'+str(testyn+1)+'.nc']
        Fn = [Fn1,Fn2,Fn3,Fn4,Fn5,Fn6,Fn7]
        print(Fn4)
        print(Fn5)
        print(Fn6)

        psi_test_input_Tr_torch, psi_test_label_Tr_torch, psi_test_label_Tr  = load_test_data(Fn,Fnmjo,leadmjo,mem_list,testyn)
        del Fn 
        psi_test_input_Tr_torch_norm = np.zeros(np.shape(psi_test_input_Tr_torch))
        ...
        for k in range(0,M):
                out = (net(psi_test_input_Tr_torch[k].reshape([1,nmaps*nmem,dimx,180]).cuda())) # Nlat changed to 1 for hovmoller forecast
        ...
        np.savetxt(path_forecasts+'predicted_UNET_7mapstrop_RMMERA5_36yr_lead'+str(leadmjo)+'_6hrnput_mem'+str(nmem)+'_'+str(testyn)+'.csv', autoreg_pred, delimiter=",")
        np.savetxt(path_forecasts+'truth_UNET_7mapstrop_RMMERA5_36yr_lead'+str(leadmjo)+'_6hrinput_mem'+str(nmem)+'_'+str(testyn)+'.csv', autoreg_true, delimiter=",")
