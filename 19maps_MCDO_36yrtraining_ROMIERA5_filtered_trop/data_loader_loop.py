...
def load_test_data(Fn,vn,Fnmjo,leadmjo,mem_list,yn,lat_lim):
    ...
    for ivn in np.arange(len(vn)):
        ...
        FF0 = FF0.sel(time=slice(str(yn)+'-01-01', str(yn+1)+'-03-01'), lat=slice(lat_lim,-lat_lim)).copy()
        # fill the Nan
        FF0.fillna(0)
    ...
    FFmjo = FFmjo.sel(time=slice(str(yn)+'-01-01', str(yn+1)+'-03-01')).copy()
    FFmjo.fillna(0)
...
def load_train_data(Fn,vn,Fnmjo,leadmjo,mem_list,yn,lat_lim):   ##### change the rest of the stuff in train
    # load training data
    # set parameters
    nmem = len(mem_list)  # memory length
    ndays = 365   # how many samples in one 'year'  
