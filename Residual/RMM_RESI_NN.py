...
noutputmem = 1 # how many time steps (days) of residual we want to forecast
...
batch_size = 64
nvar = 3 # how many variables we use as the input. nvar=2 means we use RMM from truth and prediction
# nvar=3, use RMM truth, RMM prediction and residual as the input 
...
resi_pred[:,0,None,:] = resi_pred[:,0,None,:] * std_test1 + M_test1
resi_pred[:,1,None,:] = resi_pred[:,1,None,:] * std_test2 + M_test2
...
ds.to_netcdf(path_forecasts+'residual_MCDO_UNET_'+str(nvar)+'vari_RMMERA5_36yr_lead'+str(leadmjo)+'_norm'+str(normflg)+'_dailyinput_nmem'+str(nmem)+'nout'+str(noutputmem)+'d.nc')
