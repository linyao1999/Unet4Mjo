...
nmaps = 18  # the number of variables we include in the input
...
for i in range(nmaps):
    fntemp = datadir + 'ERA5.' + vn[i] + 'GfltG.day.1901to2020.nc'
    Fn.append(fntemp)
    del fntemp
...
model_save = '/pscratch/sd/l/linyaoly/Unet4MJO/19variables_MCDO_filtered_trop_May2023/'
...
##### prepare test data ###################################################
psi_test_input_Tr_torch, psi_test_label_Tr_torch, psi_test_label_Tr, M  = load_test_data(Fn,vn[:nmaps],Fnmjo,leadmjo,mem_list,testystat,testyend,lat_lim,mjo_ind)
...
for epoch in range(0, num_epochs):  # loop over the dataset multiple times
    ...
    for yn in np.arange(ysta,yend):
        ...
        psi_train_input_Tr_torch, psi_train_label_Tr_torch = load_train_data(Fn,vn[:nmaps],Fnmjo,leadmjo,mem_list,yn,lat_lim,mjo_ind)
    ...
    #        output,_,_,_,_,_,_ = net(input_batch.cuda())
            ...
            loss.backward()
            optimizer.step()
            output_val= net(psi_test_input_Tr_torch[0:num_samples+0].reshape([num_samples,nmaps*mem_list_len,dimx,dimy]).cuda()) # Nlat changed to 1 for hovmoller forecast
            val_loss = loss_fn(output_val, psi_test_label_Tr_torch[0:num_samples+0].reshape([num_samples,2]).cuda()) # Nlat changed to 1 for hovmoller forecast
            # print statistics
...
net = net.eval()
torch.save(net.state_dict(), model_save+'predicted_MCDO_UNET_'+str(nmaps)+'maps'+str(lat_lim)+'_'+mjo_ind+'ERA5_'+str(ysta)+'to'+str(yend)+'_lead'+str(leadmjo)+'_dailyinput_nmem'+str(nmem)+'_d'+str(dmem)+'.pt')
...
ds.to_netcdf(path_forecasts+'predicted_MCDO_UNET_19maps'+str(lat_lim)+'deg_'+mjo_ind+'ERA5_lead'+str(leadmjo)+'_dailyinput_'+str(ysta)+'to'+str(yend)+'_nmem'+str(nmem)+'_d'+str(dmem)+'.nc')
