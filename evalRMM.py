...
dir = '/global/homes/l/linyaoly/ERA5/script/Stability-Explanability/7maps_36yrtraining_RMMERA5_filtered/output/'  
...
for i in np.arange(ystat, yend):
    ds = pd.read_csv('/global/homes/l/linyaoly/ERA5/reanalysis/RMM/'+str(i)+'.csv', header=0)
    RMMlist.append(ds.iloc[0:365,:])
    del ds
...
for nmem1 in ["1","30"]:
    ...
    amp_err = np.zeros(30)
    pha_err = np.zeros(30)
    tmp = np.append(np.arange(1,30), [30])
    for k,flg in zip(np.arange(len(tmp)), tmp): # lead 
        ########### read PCs in prediction and truth ################
        fplist = []
        ftlist = []
        for j in np.arange(ystat, yend):
            dsp = pd.read_csv(dir+'predicted_UNET_'+flg1+'RMMERA5_35yr_lead'+str(flg)+'_dailyinput_mem'+str(nmem1)+'d'+str(j)+'.csv', header=None, names=['RMMp1', 'RMMp2'])
            fplist.append(dsp)
            del dsp
            dst = pd.read_csv(dir+'truth_UNET_'+flg1+'RMMERA5_35yr_lead'+str(flg)+'_dailyinput_mem'+str(nmem1)+'d'+str(j)+'.csv', header=None, names=['RMMt1', 'RMMt2'])
            ftlist.append(dst)
            del dst
    ...
    eval.to_csv('./eval/UNET_'+flg1+'RMMERA5_35yr_dailyinput_mem'+str(nmem1)+'d_'+str(ystat)+'to'+str(yend)+'av.csv', index_label='lead')
    del eval
...
for i in np.arange(ystat, yend):
    ...
    for nmem1 in ["1","30"]:
        ...
        for j,flg in zip(np.arange(len(tmp)), tmp): # lead 
            ########### read PCs in prediction and truth ################
            dsp = pd.read_csv(dir+'predicted_UNET_'+flg1+'RMMERA5_35yr_lead'+str(flg)+'_dailyinput_mem'+str(nmem1)+'d'+str(i)+'.csv', header=None, names=['RMMp1', 'RMMp2'])
            dst = pd.read_csv(dir+'truth_UNET_'+flg1+'RMMERA5_35yr_lead'+str(flg)+'_dailyinput_mem'+str(nmem1)+'d'+str(i)+'.csv', header=None, names=['RMMt1', 'RMMt2'])
            ############## select strong MJO #################
            ds0 = pd.concat([ds, dsp, dst], axis=1)  # Date	PC1	PC2	IniAmp	RMMp1	RMMp2	RMMt1	RMMt2
        ...
        eval = {'BCC':BCC, 'RMSE':RMSE, 'AmpErr':amp_err, 'PhaErr':pha_err}
        eval = pd.DataFrame(data=eval,index=tmp)
        eval.to_csv('./eval/UNET_'+flg1+'RMMERA5_35yr_dailyinput_mem'+str(nmem1)+'d_'+str(i)+'.csv', index_label='lead')
        del eval
print("ds shape: " + str(ds.shape))
print("dsp shape: " + str(dsp.shape))
