...
model_save = '/pscratch/sd/l/linyaoly/Unet4MJO/19variables_MCDO_36yr_ROMI_filtered_trop/'
...
##### prepare test data ###################################################
psi_test_input_Tr_torch, psi_test_label_Tr_torch, psi_test_label_Tr  = load_test_data(Fn,vn,Fnmjo,leadmjo,mem_list,testystat,lat_lim)
...
for epoch in range(0, num_epochs):  # loop over the dataset multiple times
    ...
    for yn in np.arange(ysta,yend):
        ...
        psi_train_input_Tr_torch, psi_train_label_Tr_torch = load_train_data(Fn,vn,Fnmjo,leadmjo,mem_list,yn,lat_lim)
...
net = net.eval()
torch.save(net.state_dict(), model_save+'predicted_MCDO_UNET_'+str(len(vn))+'mapstrop_ROMIERA5_36yr_lead'+str(leadmjo)+'_dailyinput.pt')
...
for testyn in np.arange(testystat,testyend):
        ...
        Nsamp = 100
        autoreg_pred_av = np.zeros([M,2])  # (step, ROMI index)
        ...
        np.savetxt(path_forecasts+'predicted_MCDO_UNET_'+str(len(vn))+'mapstrop_ROMIERA5_36yr_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d'+str(testyn)+'.csv', autoreg_pred, delimiter=",")
        np.savetxt(path_forecasts+'truth_MCDO_UNET_'+str(len(vn))+'mapstrop_ROMIERA5_36yr_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d'+str(testyn)+'.csv', autoreg_true, delimiter=",")
        # np.savetxt(path_forecasts+'predicted_disav_MCDO_UNET_'+str(len(vn))+'mapstrop_ROMIERA5_36yr_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d'+str(testyn)+'.csv', autoreg_pred_av, delimiter=",")
        np.savetxt(path_forecasts+'predicted_disstd_MCDO_UNET_'+str(len(vn))+'mapstrop_ROMIERA5_36yr_lead'+str(leadmjo)+'_dailyinput_mem'+str(nmem)+'d'+str(testyn)+'.csv', autoreg_pred_std, delimiter=",")
